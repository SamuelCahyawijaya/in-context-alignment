...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        raise ValueError('main_icl_alignment.py <model_path_or_name> <dataset_name> <icl_type> <icl_index_type> <icl_num_exemplar> <iia_type> <iia_index_type> <iia_num_exemplar> <ioa_type> <alignment_position> <sbert_type> <batch_size>')
    ...
    SBERT_TYPE = sys.argv[11] # type of the SBERT model
    BATCH_SIZE= int(sys.argv[12])
    
    sbert_id = SBERT_TYPE.split("/")[-1]
    SAVE_NAME = f'icl-{ICL_TYPE}-{"$".join(ICL_INDEX_TYPE)}-{ICL_EXEMPLAR_COUNT}_iia-{IIA_TYPE}-{"$".join(IIA_INDEX_TYPE)}-{IIA_EXEMPLAR_COUNT}_ioa-{IOA_TYPE}_align-{ALIGN_POS}_{sbert_id}'
    os.makedirs('./metrics_icl_sbert', exist_ok=True) 
    os.makedirs('./outputs_icl_sbert', exist_ok=True) 
    ...
    for dset_lang, eval_dset in eval_dsets.items():
        ...
        if ICL_TYPE == 'cross':
            ...
            icl_indexer = XpressoDatasetIndexer(
                ...
                parallel_index_key=iia_keys,  parallel_retrieve_key=x_iia_keys, icl_index_key=icl_keys, sbert_type=SBERT_TYPE
            ) 
        else:
            icl_dset = None
            icl_indexer = None
        ...
        # Check saved data
        if exists(f'outputs_icl_sbert/icl-alignment_{DATASET_NAME}_{dset_lang}_{MODEL.split("/")[-1]}_{SAVE_NAME}.csv'):
            print("Output exist, use partial log instead")
            with open(f'outputs_icl_sbert/icl-alignment_{DATASET_NAME}_{dset_lang}_{MODEL.split("/")[-1]}_{SAVE_NAME}.csv') as csvfile:
        ...
        if len(preds) < len(eval_dset):
            for e, sample in enumerate(tqdm(eval_dset)):
                ...
                if len(preds) % (5 * BATCH_SIZE) == 0:
                    ...
                    inference_df.to_csv(f'outputs_icl_sbert/icl-alignment_{DATASET_NAME}_{dset_lang}_{MODEL.split("/")[-1]}_{SAVE_NAME}.csv', index=False)
    ...
    pd.DataFrame.from_dict(metrics).T.reset_index().to_csv(f'metrics_icl_sbert/results_{DATASET_NAME}_{MODEL.split("/")[-1]}_{SAVE_NAME}.csv', index=False)