...
if __name__ == '__main__':
    ...
    SAVE_NAME = f'itc-{ITC_TYPE}-{"$".join(ITC_INDEX_TYPE)}-{ITC_EXEMPLAR_COUNT}'
    ...
    for dset_lang, eval_dset in eval_dsets.items():
        ...
        itc_prompter = ITCPrompter(
            headers = [ # Target Inputs
                ...
                f'{lang_map[dset_lang]} Label'
            ] + [ # Source Inputs
                f'{lang_map[dset_lang]} {key.split("_")[0].capitalize()}' for key in src_keys
            ],
            target_keys=tgt_keys, label_keys=['label'], source_keys=src_keys, query_keys=query_keys
        )
